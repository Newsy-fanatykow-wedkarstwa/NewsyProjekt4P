...
const input_on_html = '<center><div class="jnputs">text: <input type="text" id="dateNewsy"> <input type="button" value="porównaj ciąg znaków" onclick="querySearch()"></div></center>';
const filtr_data = (json) => (outContent, amount) => {
    let a = 0;
    console.log(json.hits);
    outContent.innerHTML = input_on_html + "<p>ilość wyników:" + json.lenght + "</p>";
    json.forEach(element => {
        if (amount >= 1) {
            ...
            if (typeof title != 'undefined') {
                // console.log(typeof title == 'undefined', a, amount);
...
const api_filter = (query) => async (outContent, amount) => {
    let url = "https://hn.algolia.com/api/v1/search?query=" + query + "&tags=(story,job)";
    // console.log(url);
    await fetch(url)
